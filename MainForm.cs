...
namespace FullSearch
{
    public partial class MainForm : Form
    {
        ...
        {
            var mapName = txtMapName.Text.Trim();
            if (string.IsNullOrEmpty(mapName)) mapName = "sample";
            var filename = mapName + "Map.txt";
            if (!File.Exists(filename))
            {
                MessageBox.Show($"Map file '{filename}' not found. Please place it in the application's working directory.", "File not found", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                return;
            }

            try
            {
                (terrain, start, goal) = LoadMap(filename);
            }
            catch (Exception ex)
            {
            ...
            if (!Enum.TryParse<Algorithm>(algName, out var alg)) alg = Algorithm.BreadthFirst;
            var pathfinder = PathFinderFactory.NewPathFinder(alg);
            currentPath = pathfinder.FindPath(terrain, start, goal);

            // Display path in output box and save to file
            txtOutput.Clear();
            ...
            else
            {
                ...
                foreach (var c in currentPath) txtOutput.AppendText(c.ToString() + "\r\n");
                var outName = mapName + "Path_" + pathfinder.Name + ".txt";
                using var w = new StreamWriter(outName);
                foreach (var c in currentPath) w.WriteLine(c.ToString());
                if (pathfinder is AStar a) w.WriteLine(a.OpenListSorts);
